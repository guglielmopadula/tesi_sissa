...
for j in range(5):
    ...
    for i in trange(1000):
        ...
        tmp_2=np.minimum(tmp_2,np.sqrt(np.sum(cross_val_score(GaussianProcessRegressor(alpha=alpha),latent, drags, scoring=make_scorer(max_error),cv=loo)**2))/100)        
        acc=cross_val_score(GaussianProcessRegressor(alpha=alpha),latent, drags ,scoring=make_scorer(max_error),cv=loo)
        #print(np.sqrt(np.mean(cross_val_score(GaussianProcessRegressor(alpha=alpha),latent, drags, scoring=make_scorer(max_error),cv=loo))**2))
        end=time()
        times[i]=end-start  
...
np.save("err_gpr_drag.npy",all_err)
np.save("times_gpr_drag.npy",all_times)
np.save("err_gpr_drag_2.npy",all_err_2)
np.save("var_gpr_drag.npy",var_times)