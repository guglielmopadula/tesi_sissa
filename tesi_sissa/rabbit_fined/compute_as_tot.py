...
names=["data","AE","VAE","AAE","BEGAN"]

for name in names:
    NUM_SAMPLES=100
    parameters=np.load("latent_variables/"+name+"_latent.npy").reshape(600,-1)[:NUM_SAMPLES,]
    snapshot_1=np.load("physical_quantities/snaps_"+name+".npy").T.reshape(NUM_SAMPLES,-1)
    ...
    jacobian=model.predict_jacobian(parameters)[0].reshape(NUM_SAMPLES,100,-1)
    ...
    np.save("latent_variables/"+name+"_AS_latent.npy",parameters_1)
