...
for name in names:
    ...
    train_error=np.zeros_like(np.load("./rom_quantities/"+name+"_rom_err_train.npy"))
    test_error=np.zeros_like(np.load("./rom_quantities/"+name+"_rom_err_test.npy"))
    parameters=np.load("latent_variables/"+name+"_AS_latent.npy")[:NUM_SAMPLES].reshape(NUM_SAMPLES,-1).astype(np.float32)
